...
is( $Config::Tiny::VERSION, $VERSION,
    'Loaded correct version of Config::Tiny' );
...
$Trivial->{section2} = { 'this little piggy' => 'went to market' };
my $string = <<END;
root1=root2
...
# Legitimate use of UNIVERSAL::isa
ok( UNIVERSAL::isa( $Read, 'HASH' ),
    '->read of what we wrote returns a hash reference' );
...

{
   ...
   # a valid, but empty, object.
    ...
    is( scalar( keys %$Empty ),
        0, 'Config::Tiny object from empty string, is empty' );
}
...
END
    ...
    is( $Trim->{'/path/to/file.txt'}->{this},
        'that', 'First section created properly' );
